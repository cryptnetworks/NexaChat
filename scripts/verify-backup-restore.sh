...
if [[ -n "${NEXA_BACKUP_EVIDENCE_FILE:-}" ]]; then
  printf '{"status":"passed","schemaVersion":9,"accounts":2,"communities":1,"memberships":2,"spaces":1,"messages":1,"auditEvents":1,"auditCheckpoints":1,"objects":1,"elapsedSeconds":%s}\n' "$elapsed" > "$NEXA_BACKUP_EVIDENCE_FILE"
fi
