...
requiring an application restart. An outage may delay rate limits, leases, or
fan-out coordination but must never grant access or lose authoritative data.
Rollback disables the adapter; callers must retain safe database-backed
behavior. Run the opt-in real test with
