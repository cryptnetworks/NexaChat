...
Every non-operational HTTP request first consumes a global bounded route-class counter, then an address counter before its handler. A canonical community identifier in a community route consumes an independent community counter. After authentication proves that the session account matches the claimed actor, the request also consumes that account counter. Development-only actor claims use a separate `development` trust key. Operational probes and `/metrics` are excluded so an overload or Valkey outage cannot prevent orchestration from observing the process.
...
| Account (`/v1/account/*`)                          | Configured base |                      Configured base | Bounded local fallback |
| Sessions (`/v1/sessions/*`)                        |              60 |                                  120 | Fail closed with 503   |
| API tokens (`/v1/api-tokens/*`)                    |              30 |                                   60 | Fail closed with 503   |
| Invitations (any version-1 invitation route)       |              60 |                                  120 | Fail closed with 503   |
| WebSocket admission/messages (`/v1/realtime/*`)    |              60 |                                  120 | Fail closed with 503   |
| Webhooks (`/v1/webhooks/*`)                        |             120 |                                  300 | Fail closed with 503   |
| Administration (`/v1/admin/*`)                     |              30 |                                   60 | Fail closed with 503   |
...
The effective limit is the smaller of the configured base and the cap. Lower values therefore remain useful in tests and constrained deployments. Address and account scopes are independent; a request must pass both when both apply. Community limits are ten times the account policy and route-class limits are one hundred times the base policy, which bounds hostile key creation before any attacker-controlled identity key is admitted. Route keys use only the fixed classes above; raw paths and query strings never enter a key. The canonical address resolver ignores forwarding headers from untrusted peers, walks configured proxy hops from right to left, canonicalizes equivalent IPv6 forms and IPv4-mapped IPv6, and rejects ambiguous forwarding chains.

Authentication retains its identifier/source limiter as defense in depth. WebSocket upgrades consume shared route, canonical-address, and authenticated-account counters. Every WebSocket client message consumes the shared account policy as well as the connection-local command window; subscription, connection-capacity, payload, and backpressure limits remain independent lifecycle controls.
...
When coordination is intentionally disabled, a single process uses a local fixed-window store capped at 10,000 least-recently-used keys. This mode is not a shared multi-replica control. Shared route-class admission bounds attacker-created address/community keys during a window, while account and community identifiers are constrained to canonical application identifiers and every key expires. When enabled coordination becomes unavailable:

- authentication, session, API-token, invitation, WebSocket, webhook, and administrative routes reject before parsing credentials, tokens, or private content;
