...
The version-1 action allowlist is `invitation.create`, `invitation.revoke`, `invitation.accept`, `audit.checkpoint`, `audit.legal_hold.apply`, and `audit.legal_hold.release`. Outcomes are `succeeded` and `rejected`. Reason codes are machine-readable enums or bounded identifiers, never exception messages or operator prose.
