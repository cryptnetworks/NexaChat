...
describe('distributed request rate limiting', () => {
  it('classifies only bounded endpoint groups', () => {
    ...
    expect(endpointFor('GET', '/v1/sessions')).toBe('session');
    expect(endpointFor('DELETE', '/v1/api-tokens/:tokenId')).toBe('api_token');
    expect(endpointFor('POST', '/v1/webhooks/:webhookId/deliveries')).toBe(
      'webhook',
    );
    expect(endpointFor('POST', '/v1/admin/accounts/:accountId/suspend')).toBe(
      'administration',
    );
    expect(endpointFor('GET', '/v1/realtime')).toBe('websocket');
    ...
    expect(
      communityIdentityFor(
        '/v1/communities/123e4567-e89b-42d3-a456-426614174000/spaces',
      ),
    ).toBe('123e4567-e89b-42d3-a456-426614174000');
    expect(communityIdentityFor('/v1/communities/not-an-id/spaces')).toBe(
      undefined,
    );
  });

  it('rejects invalid or unsafe configuration before serving traffic', () => {
    for (const config of [
      { addressLimit: 0, accountLimit: 1, windowMs: 1 },
      { addressLimit: 1, accountLimit: 0, windowMs: 1 },
      { addressLimit: 1, accountLimit: 1, windowMs: 0 },
      { addressLimit: 1, accountLimit: 1, windowMs: 1, localBucketLimit: 0 },
    ])
      expect(() => new DistributedRequestRateLimiter(config)).toThrow(
  ...
  it('keeps attacker-controlled paths out of route keys and shares WebSocket account decisions across replicas', async () => {
    const { coordination, increment } = fakeCoordination();
    const first = new DistributedRequestRateLimiter(
      { addressLimit: 10, accountLimit: 1, windowMs: 10_000 },
      coordination,
    );
    const second = new DistributedRequestRateLimiter(
      { addressLimit: 10, accountLimit: 1, windowMs: 10_000 },
      coordination,
    );

    for (let index = 0; index < 250; index += 1)
      await first.consumeRoute({
        method: 'GET',
        route: `/v1/messages/${String(index)}?token=private-${String(index)}`,
      });
    const routeKeys = increment.mock.calls
      .map(([key]) => key)
      .filter((key) => key.startsWith('rate:route:'));
    expect(new Set(routeKeys)).toHaveLength(1);
    expect(JSON.stringify(routeKeys)).not.toContain('private-');
    ...
    await expect(first.consumeAccount(websocket)).resolves.toMatchObject({
      allowed: true,
    });
    await expect(second.consumeAccount(websocket)).resolves.toMatchObject({
      allowed: false,
      reason: 'rate_limited',
    });
  });
